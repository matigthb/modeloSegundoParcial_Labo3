...
app.post('/productos_bd', alta_baja, upload.single("foto"), (request, response) => {
...
app.post('/productos_bd/modificar', modificar, upload.single("foto"), (request, response) => {
    let file = request.file;
    let extension = mime.extension(file.mimetype);
    let obj = JSON.parse(request.body.obj);
    let path = file.destination + obj.codigo + "." + extension;
...
app.post('/productos_bd/eliminar', alta_baja, (request, response) => {
    let obj = request.body;
